...
//
_IRQL_requires_max_(DISPATCH_LEVEL) uint32_t CubeRoot(uint32_t Radicand) {
  int i;
  uint32_t x = 0;
  uint32_t y = 0;
...
_IRQL_requires_max_(DISPATCH_LEVEL) void QuicConnLogCubic(
    _In_ const QUIC_CONNECTION *const Connection) {
  (void)Connection;
}

void CubicCongestionHyStartChangeState(_In_ QUIC_CONGESTION_CONTROL *Cc,
                                       _In_ QUIC_CUBIC_HYSTART_STATE
                                           NewHyStartState) {
...
_IRQL_requires_max_(DISPATCH_LEVEL) BOOLEAN
    CubicCongestionControlCanSend(_In_ QUIC_CONGESTION_CONTROL *Cc) {
  QUIC_CONGESTION_CONTROL_CUBIC *Cubic = &Cc->Cubic;
  return Cubic->BytesInFlight < Cubic->CongestionWindow ||
         Cubic->Exemptions > 0;
}

_IRQL_requires_max_(DISPATCH_LEVEL) void CubicCongestionControlSetExemption(
    _In_ QUIC_CONGESTION_CONTROL *Cc, _In_ uint8_t NumPackets) {
  Cc->Cubic.Exemptions = NumPackets;
}

_IRQL_requires_max_(DISPATCH_LEVEL) void CubicCongestionControlReset(
...
_IRQL_requires_max_(DISPATCH_LEVEL) uint32_t
  ...
  if (Cubic->BytesInFlight >= Cubic->CongestionWindow) {
    ...
    if (Cubic->CongestionWindow < Cubic->SlowStartThreshold) {
      ...
      EstimatedWnd = Cubic->CongestionWindow +
                     (Cubic->CongestionWindow >> 2); // CongestionWindow * 1.25
    }

    SendAllowance = Cubic->LastSendAllowance +
                    (uint32_t)((EstimatedWnd * TimeSinceLastSend) /
...
_IRQL_requires_max_(DISPATCH_LEVEL) BOOLEAN
    CubicCongestionControlUpdateBlockedState(
        _In_ QUIC_CONGESTION_CONTROL *Cc, _In_ BOOLEAN PreviousCanSendState) {
  QUIC_CONNECTION *Connection = QuicCongestionControlGetConnection(Cc);
  QuicConnLogOutFlowStats(Connection);
...
_IRQL_requires_max_(DISPATCH_LEVEL) void CubicCongestionControlOnCongestionEvent(
    _In_ QUIC_CONGESTION_CONTROL *Cc, _In_ BOOLEAN IsPersistentCongestion,
    _In_ BOOLEAN Ecn) {
  QUIC_CONGESTION_CONTROL_CUBIC *Cubic = &Cc->Cubic;
  ...
  if (IsPersistentCongestion && !Cubic->IsInPersistentCongestion) {
    ...
    Connection->Paths[0].Route.State =
        RouteSuspected; // used only for RAW datapath

    Cubic->IsInPersistentCongestion = TRUE;
    Cubic->WindowPrior = Cubic->WindowMax = Cubic->WindowLastMax =
        Cubic->SlowStartThreshold = Cubic->AimdWindow =
    ...
    // (since 2^9 = 2^3^3).
    //
    Cubic->KCubic = CubeRoot(
        (Cubic->WindowMax / DatagramPayloadLength * (10 - TEN_TIMES_BETA_CUBIC)
         << 9) /
        TEN_TIMES_C_CUBIC);
    ...
    Cubic->SlowStartThreshold = Cubic->CongestionWindow = Cubic->AimdWindow =
        CXPLAT_MAX((uint32_t)DatagramPayloadLength *
                       QUIC_PERSISTENT_CONGESTION_WINDOW_PACKETS,
                   Cubic->CongestionWindow * TEN_TIMES_BETA_CUBIC / 10);
  }
}

_IRQL_requires_max_(PASSIVE_LEVEL) void CubicCongestionControlOnDataSent(
...
_IRQL_requires_max_(DISPATCH_LEVEL) BOOLEAN
    CubicCongestionControlOnDataInvalidated(_In_ QUIC_CONGESTION_CONTROL *Cc,
                                            _In_ uint32_t
...
_IRQL_requires_max_(DISPATCH_LEVEL) void CubicCongestionControlGetNetworkStatistics(
    _In_ const QUIC_CONNECTION *const Connection,
...
_IRQL_requires_max_(DISPATCH_LEVEL) BOOLEAN
    ...
    if (AckEvent->MinRttValid) {
      ...
      if (Cubic->HyStartAckCount < QUIC_HYSTART_DEFAULT_N_SAMPLING) {
        Cubic->MinRttInCurrentRound =
            CXPLAT_MIN(Cubic->MinRttInCurrentRound, AckEvent->MinRtt);
        Cubic->HyStartAckCount++;
      } else if (Cubic->HyStartState == HYSTART_NOT_STARTED) {
        const uint64_t Eta =
            CXPLAT_MIN(QUIC_HYSTART_DEFAULT_MAX_ETA,
                       CXPLAT_MAX(QUIC_HYSTART_DEFAULT_MIN_ETA,
                                  Cubic->MinRttInLastRound /
                                      8)); // Use 1/8th RTT from HyStart spec.
        //
        ...
        //
        // RTT decreased. Resume SlowStart since we assume the SlowStart exit
        // was spurious.
        //
  ...
  if (BytesAcked > 0) {
    ...
    if (Cubic->TimeOfLastAckValid) {
      const uint64_t TimeSinceLastAck =
          CxPlatTimeDiff64(Cubic->TimeOfLastAck, TimeNowUs);
      if (TimeSinceLastAck > MS_TO_US((uint64_t)Cubic->SendIdleTimeoutMs) &&
          TimeSinceLastAck > (Connection->Paths[0].SmoothedRtt +
                              4 * Connection->Paths[0].RttVariance)) {
    ...
    int64_t DeltaT =
        US_TO_MS((int64_t)TimeInCongAvoidUs - (int64_t)MS_TO_US(Cubic->KCubic) +
    ...
    int64_t CubicWindow =
        ((((DeltaT * DeltaT) >> 10) * DeltaT *
          (int64_t)(DatagramPayloadLength * TEN_TIMES_C_CUBIC / 10)) >>
         20) +
    ...
    // size as an AIMD window with BETA=0.5 and a slope (AKA ALPHA) of 1MSS/RTT.
    // Since CUBIC has BETA=0.7, we need a smaller slope than 1MSS/RTT to have
    // this property. The required slope is derived in RFC 8312 to be
    // [3*(1-BETA)/(1+BETA)]. For BETA=0.7, [3*(1-BETA)/(1+BETA)] ~= 0.5.
    //
    // This slope of 0.5MSS/RTT is used until AimdWindow reaches WindowPrior,
    // and then the slope is increased to 1MSS/RTT to match the aggressiveness
    // of Reno.
    //
    // Algorithm adapted from RFC3465 (Appropriate Byte Counting). The idea here
    // is to grow only by multiples of MTU: we record ACKed bytes in an
    // accumulator until at least a window (or two windows, if AimdWindow <
    // WindowPrior) worth of bytes are ACKed, and then increase the window by 1
    // MTU.
    //
    CXPLAT_STATIC_ASSERT(
        TEN_TIMES_BETA_CUBIC == 7,
        "TEN_TIMES_BETA_CUBIC must be 7 for simplified calculation.");
    ...
    if (Cubic->AimdWindow > CubicWindow) {
      ...
      //
      // Concave or Convex region. Constrain TargetWindow within
      // [CongestionWindow, 1.5*CongestionWindow].
      //
      uint64_t TargetWindow = CXPLAT_MAX(
          Cubic->CongestionWindow,
          CXPLAT_MIN(CubicWindow,
                     Cubic->CongestionWindow + (Cubic->CongestionWindow >> 1)));
      Cubic->CongestionWindow +=
...
Exit:
  ...
  if (Connection->Settings.NetStatsEventEnabled) {
    ...
    Event.NETWORK_STATISTICS.Bandwidth =
        Cubic->CongestionWindow / Path->SmoothedRtt;
...
_IRQL_requires_max_(DISPATCH_LEVEL) void CubicCongestionControlOnDataLost(
  ...
  if (!Cubic->HasHadCongestionEvent ||
    ...
    CubicCongestionControlOnCongestionEvent(Cc, LossEvent->PersistentCongestion,
                                            FALSE);
...
_IRQL_requires_max_(DISPATCH_LEVEL) void CubicCongestionControlOnEcn(
    _In_ QUIC_CONGESTION_CONTROL *Cc, _In_ const QUIC_ECN_EVENT *EcnEvent) {
  QUIC_CONGESTION_CONTROL_CUBIC *Cubic = &Cc->Cubic;
...
_IRQL_requires_max_(DISPATCH_LEVEL) uint8_t CubicCongestionControlGetExemptions(
    _In_ const QUIC_CONGESTION_CONTROL *Cc) {
...
_IRQL_requires_max_(DISPATCH_LEVEL) BOOLEAN
    CubicCongestionControlIsAppLimited(_In_ const QUIC_CONGESTION_CONTROL *Cc) {
  UNREFERENCED_PARAMETER(Cc);
  return FALSE;
}

_IRQL_requires_max_(DISPATCH_LEVEL) void CubicCongestionControlSetAppLimited(
...
_IRQL_requires_max_(DISPATCH_LEVEL) void CubicCongestionControlInitialize(
    _In_ QUIC_CONGESTION_CONTROL *Cc,
