...
Abstract:
    ...
    This module provides the implementation for most of the public APIs exposed
in QUIC_API_TABLE.
...
#define IS_CONN_HANDLE(Handle)                                                 \
  ((Handle) != NULL &&                                                         \
   ((Handle)->Type == QUIC_HANDLE_TYPE_CONNECTION_CLIENT ||                    \
    (Handle)->Type == QUIC_HANDLE_TYPE_CONNECTION_SERVER))
...
_IRQL_requires_max_(DISPATCH_LEVEL) QUIC_STATUS QUIC_API
    QuicConnectionOpenInPartition(
        ...
        _In_opt_ void *Context, _In_ BOOLEAN Partitioned,
        _Outptr_ _At_(*NewConnection, __drv_allocatesMem(Mem))
            _Pre_defensive_ HQUIC *NewConnection) {
  QUIC_STATUS Status;
  QUIC_REGISTRATION *Registration;
  QUIC_CONNECTION *Connection = NULL;

  if (!IS_REGISTRATION_HANDLE(RegistrationHandle) ||
      PartitionIndex >= MsQuicLib.PartitionCount || NewConnection == NULL ||
      Handler == NULL) {
...
#pragma prefast(suppress : __WARNING_25024, "Pointer cast already validated.")
  ...
  //
  Status = QuicConnAlloc(Registration, &MsQuicLib.Partitions[PartitionIndex],
  ...
  //
    ...
    !defined(CXPLAT_LINUX_XDP_ENABLED)
...
_IRQL_requires_max_(DISPATCH_LEVEL) QUIC_STATUS QUIC_API MsQuicConnectionOpen(
    _In_ _Pre_defensive_ HQUIC RegistrationHandle,
    _In_ _Pre_defensive_ QUIC_CONNECTION_CALLBACK_HANDLER Handler,
    _In_opt_ void *Context,
    _Outptr_ _At_(*NewConnection, __drv_allocatesMem(Mem))
        _Pre_defensive_ HQUIC *NewConnection) {
  return QuicConnectionOpenInPartition(RegistrationHandle,
                                       QuicLibraryGetCurrentPartition()->Index,
                                       Handler, Context, FALSE, NewConnection);
}
...
_IRQL_requires_max_(DISPATCH_LEVEL) QUIC_STATUS QUIC_API
    MsQuicConnectionOpenInPartition(
        ...
        _Outptr_ _At_(*NewConnection, __drv_allocatesMem(Mem))
            _Pre_defensive_ HQUIC *NewConnection) {
  return QuicConnectionOpenInPartition(RegistrationHandle, PartitionIndex,
                                       Handler, Context, TRUE, NewConnection);
}

#pragma warning(push)
#pragma warning(                                                               \
    disable : 6014) // SAL doesn't understand the free happens on the worker
_IRQL_requires_max_(PASSIVE_LEVEL) void QUIC_API
    MsQuicConnectionClose(_In_ _Pre_defensive_ __drv_freesMem(Mem)
                              HQUIC Handle) {
...
_IRQL_requires_max_(DISPATCH_LEVEL) void QUIC_API
    MsQuicConnectionShutdown(_In_ _Pre_defensive_ HQUIC Handle,
...
#pragma prefast(suppress : __WARNING_25024, "Pointer cast already validated.")
  ...
  if (Oper == NULL) {
    if (InterlockedCompareExchange16((short *)&Connection->BackUpOperUsed, 1,
                                     0) != 0) {
...
_IRQL_requires_max_(DISPATCH_LEVEL) QUIC_STATUS QUIC_API
    MsQuicConnectionStart(_In_ _Pre_defensive_ HQUIC Handle,
  ...
  if (ConfigHandle == NULL ||
      ConfigHandle->Type != QUIC_HANDLE_TYPE_CONFIGURATION || ServerPort == 0) {
    Status = QUIC_STATUS_INVALID_PARAMETER;
    goto Error;
  }
...
#pragma prefast(suppress : __WARNING_25024, "Pointer cast already validated.")
  ...
  if (Connection->State.Started || Connection->State.ClosedLocally) {
    Status = QUIC_STATUS_INVALID_STATE; // TODO - Support the Connect after
                                        // close/previous connect failure?
    goto Error;
  }
  ...
  if (ServerName != NULL) {
    ...
    //
#pragma prefast(suppress : __WARNING_6014,                                     \
                "Memory is correctly freed by the connection.")
    ServerNameCopy =
        CXPLAT_ALLOC_NONPAGED(ServerNameLength + 1, QUIC_POOL_SERVERNAME);
...
_IRQL_requires_max_(DISPATCH_LEVEL) QUIC_STATUS QUIC_API
    MsQuicConnectionSetConfiguration(_In_ _Pre_defensive_ HQUIC Handle,
                                     _In_ _Pre_defensive_ HQUIC ConfigHandle) {
  QUIC_STATUS Status;
  QUIC_CONNECTION *Connection;
  QUIC_CONFIGURATION *Configuration;
...
_IRQL_requires_max_(DISPATCH_LEVEL) QUIC_STATUS QUIC_API
    MsQuicConnectionSendResumptionTicket(_In_ _Pre_defensive_ HQUIC Handle,
...
#pragma prefast(suppress : __WARNING_25024, "Pointer cast already validated.")
  ...
  if (!Connection->State.ResumptionEnabled || !Connection->State.Connected ||
      !Connection->Crypto.TlsState.HandshakeComplete) {
    Status = QUIC_STATUS_INVALID_STATE; // TODO - Support queueing up the ticket
  ...
  if (DataLength > 0) {
    ResumptionDataCopy =
        CXPLAT_ALLOC_NONPAGED(DataLength, QUIC_POOL_APP_RESUMPTION_DATA);
  ...
  Oper->API_CALL.Context->CONN_SEND_RESUMPTION_TICKET.ResumptionAppData =
      ResumptionDataCopy;
  Oper->API_CALL.Context->CONN_SEND_RESUMPTION_TICKET.AppDataLength =
      DataLength;
...
#pragma prefast(suppress : __WARNING_25024, "Pointer cast already validated.")
  ...
  Status =
      QuicStreamInitialize(Connection, FALSE, Flags, (QUIC_STREAM **)NewStream);
  if (QUIC_FAILED(Status)) {
    goto Error;
  }
...
#pragma warning(                                                               \
    disable : 6014) // SAL doesn't understand the free happens on the worker
_IRQL_requires_max_(PASSIVE_LEVEL) void QUIC_API
    MsQuicStreamClose(_In_ _Pre_defensive_ __drv_freesMem(Mem) HQUIC Handle) {
  QUIC_STREAM *Stream;
  QUIC_CONNECTION *Connection;
...
_IRQL_requires_max_(DISPATCH_LEVEL) QUIC_STATUS QUIC_API
    MsQuicStreamStart(_In_ _Pre_defensive_ HQUIC Handle,
...
_IRQL_requires_max_(DISPATCH_LEVEL) QUIC_STATUS QUIC_API
    MsQuicStreamShutdown(_In_ _Pre_defensive_ HQUIC Handle,
  ...
  if (!IS_STREAM_HANDLE(Handle) || Flags == 0 ||
  ...
  if (Flags & QUIC_STREAM_SHUTDOWN_FLAG_GRACEFUL &&
      Flags & (QUIC_STREAM_SHUTDOWN_FLAG_ABORT |
...
_IRQL_requires_max_(DISPATCH_LEVEL) QUIC_STATUS QUIC_API
    MsQuicStreamSend(_In_ _Pre_defensive_ HQUIC Handle,
...
#pragma prefast(suppress : __WARNING_6014,                                     \
                "Memory is correctly freed (QuicStreamCompleteSendRequest).")
...
#pragma warning(push)
#pragma warning(disable : 6240) // CXPLAT_AT_DISPATCH only really does anything
                                // for kernel mode
  SendInline = !Connection->Settings.SendBufferingEnabled &&
...
#pragma warning(pop)
  ...
  if (!Stream->Flags.SendEnabled) {
    ...
    while (*ApiSendRequestsTail != NULL) {
      ...
      QueueOper =
          FALSE; // Not necessary if the previous send hasn't been flushed yet.
    }
  ...
  if (SendInline) {
    ...
    if (Oper == NULL) {
      ...
      Oper->API_CALL.Context->CONN_SHUTDOWN.Flags =
          ...
          QUIC_CONNECTION_SHUTDOWN_FLAG_STATUS;
      Oper->API_CALL.Context->CONN_SHUTDOWN.ErrorCode =
          (QUIC_VAR_INT)QUIC_STATUS_OUT_OF_MEMORY;
      Oper->API_CALL.Context->CONN_SHUTDOWN.RegistrationShutdown = FALSE;
...
_IRQL_requires_max_(DISPATCH_LEVEL) QUIC_STATUS QUIC_API
    MsQuicStreamReceiveSetEnabled(_In_ _Pre_defensive_ HQUIC Handle,
...
_IRQL_requires_max_(DISPATCH_LEVEL) void QUIC_API
    MsQuicStreamReceiveComplete(_In_ _Pre_defensive_ HQUIC Handle,
...
#pragma prefast(suppress : __WARNING_25024, "Pointer cast already validated.")
  ...
  QUIC_CONN_VERIFY(Connection,
                   (Stream->RecvPendingLength ==
                    0) || // Stream might have been shutdown already
  ...
  if ((BufferLength & QUIC_STREAM_RECV_COMPLETION_LENGTH_CANARY_BIT) != 0 &&
      (RecvCompletionLength & QUIC_STREAM_RECV_COMPLETION_LENGTH_CANARY_BIT) !=
          0) {
    ...
    Oper->API_CALL.Context->CONN_SHUTDOWN.Flags =
        ...
        QUIC_CONNECTION_SHUTDOWN_FLAG_STATUS;
    Oper->API_CALL.Context->CONN_SHUTDOWN.ErrorCode =
        (QUIC_VAR_INT)QUIC_STATUS_INVALID_STATE;
    Oper->API_CALL.Context->CONN_SHUTDOWN.RegistrationShutdown = FALSE;
    Oper->API_CALL.Context->CONN_SHUTDOWN.TransportShutdown = TRUE;
    QuicConnQueueHighestPriorityOper(Connection, Oper);
  }

  Oper = InterlockedFetchAndClearPointer(
...
_IRQL_requires_max_(DISPATCH_LEVEL) QUIC_STATUS QUIC_API
    MsQuicStreamProvideReceiveBuffers(_In_ _Pre_defensive_ HQUIC Handle,
...
#pragma prefast(suppress : __WARNING_25024, "Pointer cast already validated.")
  ...
  if (IsWorkerThread) {
    ...
    //
    // Queue the operation to insert the chunks in the recv buffer, without
    ...
    CxPlatListInitializeHead(
        &Oper->API_CALL.Context->STRM_PROVIDE_RECV_BUFFERS.Chunks);
    CxPlatListMoveItems(
        &ChunkList, &Oper->API_CALL.Context->STRM_PROVIDE_RECV_BUFFERS.Chunks);
...
Error:
  ...
  while (!CxPlatListIsEmpty(&ChunkList)) {
    ...
    CxPlatPoolFree(CXPLAT_CONTAINING_RECORD(CxPlatListRemoveHead(&ChunkList),
                                            QUIC_RECV_CHUNK, Link));
    CXPLAT_FREE(CXPLAT_CONTAINING_RECORD(CxPlatListRemoveHead(&ChunkList),
                                         QUIC_RECV_CHUNK, Link),
                QUIC_POOL_RECVBUF);
  }
...
_IRQL_requires_max_(PASSIVE_LEVEL) QUIC_STATUS QUIC_API
    MsQuicSetParam(_When_(QUIC_PARAM_IS_GLOBAL(Param), _Reserved_)
                       _When_(!QUIC_PARAM_IS_GLOBAL(Param),
...
_IRQL_requires_max_(PASSIVE_LEVEL) QUIC_STATUS QUIC_API
    MsQuicGetParam(_When_(QUIC_PARAM_IS_GLOBAL(Param), _Reserved_)
                       _When_(!QUIC_PARAM_IS_GLOBAL(Param),
                              _In_ _Pre_defensive_) HQUIC Handle,
                   _In_ uint32_t Param,
  ...
  if ((Handle == NULL) ^ QUIC_PARAM_IS_GLOBAL(Param) || BufferLength == NULL) {
    //
    // Ensure global parameters don't have a handle passed in, and vice
    // versa.
    //
...
_IRQL_requires_max_(DISPATCH_LEVEL) QUIC_STATUS QUIC_API
    MsQuicDatagramSend(_In_ _Pre_defensive_ HQUIC Handle,
                       _In_reads_(BufferCount)
                           _Pre_defensive_ const QUIC_BUFFER *const Buffers,
                       _In_ uint32_t BufferCount, _In_ QUIC_SEND_FLAGS Flags,
                       _In_opt_ void *ClientSendContext) {
...
_IRQL_requires_max_(DISPATCH_LEVEL) QUIC_STATUS QUIC_API
    MsQuicConnectionResumptionTicketValidationComplete(
        _In_ _Pre_defensive_ HQUIC Handle, _In_ BOOLEAN Result) {
  QUIC_STATUS Status;
  QUIC_CONNECTION *Connection;
  QUIC_OPERATION *Oper;
...
#pragma prefast(suppress : __WARNING_25024, "Pointer cast already validated.")
  ...
  Oper->API_CALL.Context->CONN_COMPLETE_RESUMPTION_TICKET_VALIDATION.Result =
      Result;
...
#pragma prefast(suppress : __WARNING_25024, "Pointer cast already validated.")
  ...
  Oper->API_CALL.Context->CONN_COMPLETE_CERTIFICATE_VALIDATION.TlsAlert =
      TlsAlert;
