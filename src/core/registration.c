...
void QuicRegistrationClose(_Inout_ QUIC_REGISTRATION *Registration) {
...
#if DEBUG
  CXPLAT_DBG_ASSERT(!CxPlatRefDecrement(
      &Registration->RefTypeBiasedCount[QUIC_REG_REF_HANDLE_OWNER]));
#endif
  ...
  for (uint32_t i = 0; i < QUIC_REG_REF_COUNT; i++) {
    ...
    //
    CXPLAT_DBG_ASSERT(
        QuicReadLongPtrNoFence(&Registration->RefTypeBiasedCount[i]) == 1);
  }
#endif
...
CXPLAT_THREAD_CALLBACK(RegistrationCloseWorker, Context) {
  ...
  if (Registration->CloseCompleteHandler != NULL) {
    ...
    CXPLAT_FRE_ASSERTMSG(
        ...
        "The library is being unloaded while a registration is not fully "
        "closed!");
    ...
    WakeCleanupWorker =
        CxPlatListIsEmpty(&MsQuicLib.RegistrationCloseCleanupList);
    CxPlatListInsertTail(&MsQuicLib.RegistrationCloseCleanupList,
                         &Registration->CloseCleanupEntry);
...
_IRQL_requires_max_(PASSIVE_LEVEL) QUIC_STATUS QUIC_API
    MsQuicRegistrationOpen(_In_opt_ const QUIC_REGISTRATION_CONFIG *Config,
                           _Outptr_ _At_(*NewRegistration,
                                         __drv_allocatesMem(Mem))
                               _Pre_defensive_ HQUIC *NewRegistration) {
  QUIC_STATUS Status;
  ...
  //
  // For external registrations, we need to take the library lock. For the
  // internal registration, the caller of this function holds the lock.
  //
  ...
  Registration =
      CXPLAT_ALLOC_NONPAGED(RegistrationSize, QUIC_POOL_REGISTRATION);
  ...
  Registration->ExecProfile = Config == NULL
...
#if DEBUG
  CxPlatRefInitializeMultiple(Registration->RefTypeBiasedCount,
                              QUIC_REG_REF_COUNT);
  CxPlatRefIncrement(
      &Registration->RefTypeBiasedCount[QUIC_REG_REF_HANDLE_OWNER]);
  QuicLibraryTrackDbgObject(QUIC_DBG_OBJECT_TYPE_REGISTRATION,
                            &Registration->DbgObjectLink);
#endif
  ...
  Status = QuicWorkerPoolInitialize(Registration, Registration->ExecProfile,
  ...
  CXPLAT_THREAD_CONFIG ThreadConfig = {
      0, 0, "RegistrationCloseWorker", RegistrationCloseWorker, Registration,
  };
...
Error:
  ...
  if (Registration != NULL) {
    ...
    CxPlatRefDecrement(
        &Registration->RefTypeBiasedCount[QUIC_REG_REF_HANDLE_OWNER]);
    QuicLibraryUntrackDbgObject(QUIC_DBG_OBJECT_TYPE_REGISTRATION,
                                &Registration->DbgObjectLink);
#endif
    CxPlatRundownUninitialize(&Registration->Rundown);
    CxPlatDispatchLockUninitialize(&Registration->ConnectionLock);
...
_IRQL_requires_max_(PASSIVE_LEVEL) void QUIC_API
    MsQuicRegistrationClose(_In_ _Pre_defensive_ __drv_freesMem(Mem)
                                HQUIC Handle) {
...
_IRQL_requires_max_(PASSIVE_LEVEL) void QUIC_API MsQuicRegistrationClose2(
    _In_ _Pre_defensive_ __drv_freesMem(Mem) HQUIC Handle,
...
_IRQL_requires_max_(DISPATCH_LEVEL) void QUIC_API
    MsQuicRegistrationShutdown(_In_ _Pre_defensive_ HQUIC Handle,
...
_IRQL_requires_max_(PASSIVE_LEVEL) void QuicRegistrationTraceRundown(
    _In_ QUIC_REGISTRATION *Registration) {

  CxPlatLockAcquire(&Registration->ConfigLock);

  for (CXPLAT_LIST_ENTRY *Link = Registration->Configurations.Flink;
       Link != &Registration->Configurations; Link = Link->Flink) {
  ...
  for (CXPLAT_LIST_ENTRY *Link = Registration->Connections.Flink;
       Link != &Registration->Connections; Link = Link->Flink) {
...
_IRQL_requires_max_(PASSIVE_LEVEL) void QuicRegistrationSettingsChanged(
...
_IRQL_requires_max_(DISPATCH_LEVEL) BOOLEAN
    QuicRegistrationAcceptConnection(_In_ QUIC_REGISTRATION *Registration,
                                     _In_ QUIC_CONNECTION *Connection) {
  const uint16_t Index = Registration->NoPartitioning
                             ? 0
...
_IRQL_requires_max_(DISPATCH_LEVEL) void QuicRegistrationQueueNewConnection(
    _In_ QUIC_REGISTRATION *Registration, _In_ QUIC_CONNECTION *Connection) {
  uint16_t Index = Registration->NoPartitioning
                       ? 0
                       : QuicPartitionIdGetIndex(Connection->PartitionID);
  ...
  QuicWorkerAssignConnection(&Registration->WorkerPool->Workers[Index],
                             Connection);
}

_IRQL_requires_max_(PASSIVE_LEVEL) QUIC_STATUS
    QuicRegistrationParamSet(_In_ QUIC_REGISTRATION *Registration,
...
_IRQL_requires_max_(PASSIVE_LEVEL) QUIC_STATUS QuicRegistrationParamGet(
    _In_ QUIC_REGISTRATION *Registration, _In_ uint32_t Param,
