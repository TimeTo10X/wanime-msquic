...
void CxPlatSystemLoad(void) {
  ...
  if (numa_available() >= 0) {
    ...
    CxPlatNumaNodeMasks = CXPLAT_ALLOC_NONPAGED(
        sizeof(cpu_set_t) * CxPlatNumaNodeCount, QUIC_POOL_PLATFORM_PROC);
    CXPLAT_FRE_ASSERT(CxPlatNumaNodeMasks);
    for (uint32_t n = 0; n < CxPlatNumaNodeCount; ++n) {
      CPU_ZERO(&CxPlatNumaNodeMasks[n]);
      CXPLAT_FRE_ASSERT(numa_node_to_cpus_compat((int)n,
                                                 CxPlatNumaNodeMasks[n].__bits,
                                                 sizeof(cpu_set_t)) >= 0);
    }
  } else {
  ...
  //
  // Find the length of the full path without the shared object name, including
  // the trailing slash.
  //
  ...
  char *ProviderFullPath =
      CXPLAT_ALLOC_PAGED(ProviderFullPathLength, QUIC_POOL_PLATFORM_TMP_ALLOC);
  if (ProviderFullPath == NULL) {
    goto Exit;
  }

  CxPlatCopyMemory(ProviderFullPath, Info.dli_fname, LastTrailingSlashLen);
  CxPlatCopyMemory(ProviderFullPath + LastTrailingSlashLen, TpLibName,
                   TpLibNameLen);
  ProviderFullPath[LastTrailingSlashLen + TpLibNameLen] = '\0';

  //
  // Load the tracepoint provider.
  // It's OK if this fails - that just means that tracing dependencies aren't
  // available.
  //
...
void *CxPlatAlloc(_In_ size_t ByteCount, _In_ uint32_t Tag) {
  ...
  uint32_t Rand;
  if ((CxPlatform.AllocFailDenominator > 0 &&
       (CxPlatRandom(sizeof(Rand), &Rand),
        Rand % CxPlatform.AllocFailDenominator) == 1) ||
      (CxPlatform.AllocFailDenominator < 0 &&
       InterlockedIncrement(&CxPlatform.AllocCounter) %
               CxPlatform.AllocFailDenominator ==
           0)) {
    return NULL;
  }
#endif
...
void CxPlatFree(__drv_freesMem(Mem) _Frees_ptr_ void *Mem, _In_ uint32_t Tag) {
  UNREFERENCED_PARAMETER(Tag);
  free(Mem);
}

void CxPlatRefInitialize(_Inout_ CXPLAT_REF_COUNT *RefCount) { *RefCount = 1; }

void CxPlatRefInitializeEx(_Inout_ CXPLAT_REF_COUNT *RefCount,
                           _In_ uint32_t Initial) {
  *RefCount = (int64_t)Initial;
}

void CxPlatRefInitializeMultiple(_Out_writes_(Count)
                                     CXPLAT_REF_COUNT *RefCounts,
                                 _In_ uint32_t Count) {
...
void CxPlatRefIncrement(_Inout_ CXPLAT_REF_COUNT *RefCount) {
...
BOOLEAN
CxPlatRefIncrementNonZero(_Inout_ volatile CXPLAT_REF_COUNT *RefCount,
                          _In_ uint32_t Bias) {
  ...
  for (;;) {
    ...
    if (NewValue > Bias) {
      if (__atomic_compare_exchange_n(RefCount, &OldValue, NewValue, false,
...
BOOLEAN
CxPlatRefDecrement(_In_ CXPLAT_REF_COUNT *RefCount) {
...
void CxPlatRundownInitialize(_Inout_ CXPLAT_RUNDOWN_REF *Rundown) {
  CxPlatRefInitialize(&((Rundown)->RefCount));
  CxPlatEventInitialize(&((Rundown)->RundownComplete), false, false);
}

void CxPlatRundownInitializeDisabled(_Inout_ CXPLAT_RUNDOWN_REF *Rundown) {
  (Rundown)->RefCount = 0;
  CxPlatEventInitialize(&((Rundown)->RundownComplete), false, false);
}

void CxPlatRundownReInitialize(_Inout_ CXPLAT_RUNDOWN_REF *Rundown) {
  (Rundown)->RefCount = 1;
}

void CxPlatRundownUninitialize(_Inout_ CXPLAT_RUNDOWN_REF *Rundown) {
  CxPlatEventUninitialize((Rundown)->RundownComplete);
}

BOOLEAN
CxPlatRundownAcquire(_Inout_ CXPLAT_RUNDOWN_REF *Rundown) {
  return CxPlatRefIncrementNonZero(&(Rundown)->RefCount, 1);
}

void CxPlatRundownRelease(_Inout_ CXPLAT_RUNDOWN_REF *Rundown) {
  if (CxPlatRefDecrement(&(Rundown)->RefCount)) {
    CxPlatEventSet((Rundown)->RundownComplete);
  }
}

void CxPlatRundownReleaseAndWait(_Inout_ CXPLAT_RUNDOWN_REF *Rundown) {
  if (!CxPlatRefDecrement(&(Rundown)->RefCount)) {
    CxPlatEventWaitForever((Rundown)->RundownComplete);
  }
}

uint64_t CxPlatTimespecToUs(_In_ const struct timespec *Time) {
  return ((uint64_t)Time->tv_sec * CXPLAT_MICROSEC_PER_SEC) +
         ((uint64_t)Time->tv_nsec / CXPLAT_NANOSEC_PER_MICROSEC);
}
...
void CxPlatGetAbsoluteTime(_In_ unsigned long DeltaMs,
...
void CxPlatSleep(_In_ uint32_t DurationMs) {
  int ErrorCode = 0;
  struct timespec TS = {
...
QUIC_STATUS
CxPlatRandom(_In_ uint32_t BufferLen,
...
void CxPlatConvertToMappedV6(_In_ const QUIC_ADDR *InAddr,
  ...
  if (InAddr->Ip.sa_family == QUIC_ADDRESS_FAMILY_INET) {
    ...
    memcpy(&(OutAddr->Ipv6.sin6_addr.s6_addr[12]),
           &InAddr->Ipv4.sin_addr.s_addr, 4);
  } else {
    *OutAddr = *InAddr;
  }
}

void CxPlatConvertFromMappedV6(_In_ const QUIC_ADDR *InAddr,
                               _Out_ QUIC_ADDR *OutAddr) {
  ...
  if (IN6_IS_ADDR_V4MAPPED(&InAddr->Ipv6.sin6_addr)) {
    ...
    memcpy(&TmpAddr->Ipv4.sin_addr.s_addr, &InAddr->Ipv6.sin6_addr.s6_addr[12],
           4);
    *OutAddr = *TmpAddr;
...
#ifdef DEBUG
void CxPlatSetAllocFailDenominator(_In_ int32_t Value) {
  CxPlatform.AllocFailDenominator = Value;
  CxPlatform.AllocCounter = 0;
}

int32_t CxPlatGetAllocFailDenominator() {
...
QUIC_STATUS
CxPlatThreadCreate(_In_ CXPLAT_THREAD_CONFIG *Config,
  ...
  CXPLAT_THREAD_CUSTOM_CONTEXT *CustomContext = CXPLAT_ALLOC_NONPAGED(
      sizeof(CXPLAT_THREAD_CUSTOM_CONTEXT), QUIC_POOL_CUSTOM_THREAD);
  ...
  //
  // If pthread_create fails with an error code, then try again without the
  ...
  if (Status == QUIC_STATUS_SUCCESS) {
    if (Config->Flags & CXPLAT_THREAD_FLAG_SET_AFFINITIZE) {
      ...
      if (pthread_setaffinity_np(*Thread, sizeof(cpu_set_t),
                                 &CxPlatNumaNodeMasks[IdealNumaNode])) {
      }
#endif
    }
  }
...
void CxPlatThreadDelete(_Inout_ CXPLAT_THREAD *Thread) {
  UNREFERENCED_PARAMETER(Thread);
}

void CxPlatThreadWait(_Inout_ CXPLAT_THREAD *Thread) {
  CXPLAT_DBG_ASSERT(pthread_equal(*Thread, pthread_self()) == 0);
  CXPLAT_FRE_ASSERT(pthread_join(*Thread, NULL) == 0);
}

CXPLAT_THREAD_ID
CxPlatCurThreadID(void) {
...
// For FreeBSD
  ...
  CXPLAT_STATIC_ASSERT(sizeof(pid_t) <= sizeof(CXPLAT_THREAD_ID),
                       "PID size exceeds the expected size");
  return syscall(SYS_gettid);

#elif defined(CX_PLATFORM_DARWIN)
  // cppcheck-suppress duplicateExpression
  CXPLAT_STATIC_ASSERT(sizeof(uint32_t) == sizeof(CXPLAT_THREAD_ID),
                       "The cast depends on thread id being 32 bits");
  uint64_t Tid;
...
void CxPlatLogAssert(_In_z_ const char *File, _In_ int Line,
                     _In_z_ const char *Expr) {
